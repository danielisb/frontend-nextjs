...
export default function CicloDeVida({ initialCount }: CicloDeVidaProps) {
  ...
  const handleComplete = () => {
    ...
    console.log('Counter chegou a 10 e foi desmontado');
  };

  useEffect(() => {
    const onMountListener = () => console.log('Evento capturado: onCounterMount');
    const onUnmountListener = () => console.log('Evento capturado: onCounterUnmount');
    const onUpdateListener = (event: CustomEventInit) =>
      console.log('Evento capturado: onCounterUpdate', event.detail);

    window.addEventListener('onCounterMount', onMountListener);
    window.addEventListener('onCounterUnmount', onUnmountListener);
    window.addEventListener('onCounterUpdate', onUpdateListener);

    return () => {
      window.removeEventListener('onCounterMount', onMountListener);
      window.removeEventListener('onCounterUnmount', onUnmountListener);
      window.removeEventListener('onCounterUpdate', onUpdateListener);
    };
  }, []);
